...
createWifiConfigList () {
	local option="$1"
	local count=0
	local resp=""
	# check that wifi-config section exists
	local exists=$(uci -q get wireless.@wifi-config[$count])

	while [ "$exists" != "" ]; do
		val=$(uci -q get wireless.@wifi-config[$count].$option)
		if [ "$val" == "" ]; then
			resp="$resp /"
		else
			resp="$resp$val/"
		...
		count=$(($count+1))
		exists=$(uci -q get wireless.@wifi-config[$count])
	done
...
ralink_setup_sta(){
	...
	device_mode="$(uci get wireless.radio0.device_mode)"
	[ "${device_mode}" == "ap" ] && return
	[ "${device_mode}" == "apsta" ] && hide=0
	...
	ssid_list=$(createWifiConfigList "ssid")
	pass_list=$(createWifiConfigList "key")
	bssid_list=$(createWifiConfigList "bssid")

	op_mode="$(uci -q get wireless.radio0.op_mode)"
	if [ "$op_mode" == "" ]; then
		op_mode="preference"
	fi
...
EOF
	...
	device_mode="$(uci get wireless.radio0.device_mode)"
	[ "${device_mode}" = "ap" -a -n "${LED}" -a -f /sys/class/leds/${LED}/trigger ] && ap_client ${LED} set
}
